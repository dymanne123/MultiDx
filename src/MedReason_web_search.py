...
def read_jsonl_line(filepath, line_number):
...
def generate_and_save_prompts(data_test, folder_path, num_shot,args):
    ...
    file_paths = []
    samples = []
    agent = create_agent(model_id="deepseek/deepseek-reasoner")
    for i in tqdm(range(len(data_test))):
        ...
        sample = data_test[i]
        thought = read_jsonl_line("generated_predictions.jsonl", i)
        """
        cur_prompt = my_generate_prompt_CoT(  
            sample['case_prompt'],num_shot,thought
        )
        """
        ...
        if len(input_prompts) >= batch_size:
            run_one_batch_deep(agent,input_prompts, samples, file_paths)
...
def my_generate_prompt_ICL(case_prompt,num_shot):
    ...
    return prompt
def my_generate_prompt_CoT(case_prompt,num_shot,thought):
    '''
    Gnerate the prompt for the model
...
def run_one_batch_deep(agent,input_prompts, samples, file_paths, max_new_tokens=512):
    '''
    Generate the completion for one batch of input prompts
    ...
    for j in range(len(input_prompts)):
        ...
        try:
            ...
            cur_sample.update({'prediction': response })
...
def main():
    ...
    folder_path = f'../results/{dataset_name}_{args.model}_10shot_deep_research'
